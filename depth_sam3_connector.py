...
def main():
    parser = argparse.ArgumentParser(description="Depth + SAM3 Spatial Analysis")

    # Input options
    parser.add_argument("--image", "-i", required=True, help="Image path")
    parser.add_argument("--prompts", "-p", nargs="+", required=True, help="Text prompts")
    ...
    graph = connector.analyze(image, args.prompts, args.image)
